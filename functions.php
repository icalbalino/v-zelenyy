   ...
   function select_items($id="") {
    ...
    function selectTrxes($kasir_id = ""){
        ...
        if($kasir_id!=""){
            ...
            $sql = "SELECT trx.id, trx.kasir_id, user.nama, trx.tanggal, sum(subtotal) as 'total' FROM trx join `detail_trx` join `user` where user.id = trx.kasir_id and detail_trx.trx_id = trx.id and user.role = 'kasir' group by trx.id";
        }
        try {
            $stmt = $con->prepare($sql);
